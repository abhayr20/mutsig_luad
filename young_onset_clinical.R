...
##TCGA-LUAD
tcga_smoker <- read.delim("clinical_data/tcga_luad/exposure.tsv", header = TRUE,
                          sep = "\t", fill = TRUE, check.names = FALSE) %>%
  select(case_submitter_id, cigarettes_per_day, pack_years_smoked) %>%
  rename(patient_id = case_submitter_id,
         daily_cigs = cigarettes_per_day,
         pack_years = pack_years_smoked) %>%
  mutate(across(everything(), ~ na_if(., "'--"))) %>%
  mutate(smoking_status = if_else(is.na(daily_cigs), "non_smoker", "smoker")) %>%
  select(patient_id, smoking_status)

# Read and preprocess the tcga_luad dataframe
tcga_luad <- read.delim("tcga_luad/data_clinical_patient.txt", skip = 4, header = TRUE) %>%
  ...
  mutate(race = case_when(
    ancestry == "EUR" ~ "White",
    ancestry == "AFR" ~ "Black or African American",
    ancestry == "EAS" ~ "Asian",
    ancestry == " " ~ NA,
    TRUE ~ NA
  ))

# Join tcga_luad with tcga_smoker to update smoking_status
tcga_luad <- tcga_luad %>%
  left_join(tcga_smoker, by = "patient_id") %>%
  mutate(smoking_status = coalesce(smoking_status.y, smoking_status.x)) %>%
  select(-smoking_status.x, -smoking_status.y)
